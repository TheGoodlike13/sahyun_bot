...
def test_api_accessible(twitchy):
    # assert_that(twitchy.is_following(streamer=13144519, viewer=37103864)).is_true()  deleted account
    # assert_that(twitchy.is_following(streamer='sahyun', viewer='thegoodlike13')).is_true()  deleted account
...
def test_user_id(twitchy):
    ...
    # assert_that(twitchy.get_id('thegoodlike13')).is_equal_to(37103864)  deleted account
    assert_that(twitchy.get_id('_not_possible')).is_none()


def test_hosts(twitchy):
    with HTTMock(twitch_hosts):
        # assert_that(twitchy.hosts('thegoodlike13')).is_empty()  deleted account
        # assert_that(twitchy.hosts('sahyun')).contains_only('thegoodlike13')  deleted account
        pass
