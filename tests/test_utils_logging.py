...
def small():
    with HTTMock(simple):
        return requests.post('http://localhost/small', data={'password': 'secret'}, headers={'request_header': 'out'})


@pytest.fixture
def big():
    with HTTMock(simple):
        return requests.post('http://localhost/big', data={'password': 'secret'}, headers={'request_header': 'out'})
